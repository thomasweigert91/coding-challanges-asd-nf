...
function comp(a1: number[] | null, a2: number[] | null): boolean {
  ...
  const squaredValues = a1.map((value) => value ** 2);
  const sortedA1 = squaredValues.sort((a, b) => a - b);
  const sortedA2 = [...a2].sort((a, b) => a - b);

  for (let index = 0; index < sortedA1.length; index++) {
    if (sortedA1[index] !== sortedA2[index]) {
      return false;
    }
  }
